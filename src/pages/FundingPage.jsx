  ...
  if (loading) {
    return (
      <div className="py-24 flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-sm font-semibold text-slate-500">Loading Smart Financial Planner...</p>
      </div>
    );
  }
      ...
      <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200/90 shadow-soft-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-50 text-emerald-800 border border-emerald-200 text-xs font-bold mb-2">
            ...
            <span>AI Hyper-Local Financial Structuring Assistant (PS26091)</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">
            Smart Funding Planner
          </h1>
          <p className="text-sm text-slate-500 mt-1 max-w-2xl leading-relaxed">
            Understand how much you may be able to structure, which funding option fits your project, and what repayment could look like.
          </p>
        </div>

        <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-left shrink-0 flex items-center gap-4">
          <div>
            <span className="text-[10px] uppercase font-bold text-slate-400 block">Indicative Feasible Outlay</span>
          ...
          <div>
            <span className="text-[10px] uppercase font-bold text-emerald-700 block">Potential Financing</span>
      ...
      <div className="bg-gradient-to-r from-slate-900 to-slate-800 text-white rounded-3xl p-6 sm:p-8 shadow-soft-md flex flex-col md:flex-row md:items-center justify-between gap-5">
        <div className="space-y-1.5 max-w-2xl">
          <div className="inline-flex items-center gap-1.5 text-xs font-bold text-emerald-400 bg-emerald-950/80 px-2.5 py-0.5 rounded-full border border-emerald-800">
            ...
            <span>🎯 Next Financial Milestone</span>
          </div>
          <h3 className="text-xl font-black text-white tracking-tight">
            Prepare your Detailed Project Report (DPR) & Quotations
          </h3>
          <p className="text-xs text-slate-300 leading-relaxed">
            Your funding requirement has been calculated at <strong>{formatRupees(structure.potentialLoan)}</strong> under the {structure.product?.name || 'Scheme Tier'}. The next recommended action is consolidating vendor machinery quotations and registering on the official Udyam portal to unlock bank appraisal.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 shrink-0">
          ...
          >
            <span>View Financial Roadmap</span>
